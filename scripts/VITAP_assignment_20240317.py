...
def multipartite_taxonomic_graph_generator(result_dir, vmr_mapping_file, taxon_categories):
	...
	def process_row(row):
		...
		# If all weight values are less than 0.3
		for i in range(len(weights)):
			average = row[weights[-(i+1):]].mean()
			if pd.notna(average):
				lineage = ";".join(row[lineages[-(i+1):]])
				return pd.Series([average, lineage])

		return pd.Series(['NaN', 'NaN'])

	# Complete the lineage column
	...
	print('[INFO] Importing taxonomic tripartite graphs and ICTV taxonomic hierarchy')
	# read txonomic file
	species_annotation_file = os.path.join(result_dir, "Species_tripartite.graph")
	species_df = pd.read_csv(species_annotation_file, sep='\t')

	# read the taxonomic hierarchy file
	vmr_df = pd.read_csv(vmr_mapping_file)

	merged_df = species_df.merge(vmr_df[['Species', 'Genus', 'Family', 'Order', 'Class', 'Phylum', 'Kingdom', 'Realm']], 
								 left_on='Species', right_on='Species', how='left')

	# add empty weight columns after each taxonomic hierarchy level
	...
	# merge the weight data for each taxonomic hierarchy level
	for taxa in taxon_categories[1:]:
		unit_annotation_file = os.path.join(result_dir, f"{taxa}_tripartite.graph")
		unit_df = pd.read_csv(unit_annotation_file, sep='\t')
		merged_df = merged_df.merge(unit_df[['Genome_ID', taxa, f'{taxa}_weight']], 
									on=['Genome_ID', taxa], 
									how='left', 
									suffixes=('', f'_from_{taxa}'))
	...
	# define the weight column and the corresponding taxonomic level column
	...
	merged_df[['lineage_score', 'lineage']] = merged_df.apply(process_row, axis=1)
	...
	conditions = [
		...
		best_lineage_df['lineage_score'] >= 0.1,
	]
...
def assignment():		 
	...
	for taxa in taxon_categories:   
		...
		taxa_annot_file = os.path.join(result_folder, f"{taxa}_tripartite.graph")
