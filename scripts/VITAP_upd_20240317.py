...
def upd():
	...
	# retrieve the set of downloaded IDs
	...
	# process the file that requires sequence truncation
	for row in rows:
		virus_id = row[0]
		start_end_sites = row[-1]
	...
	# ===== get the current date and generate a new folder name =====
	today = datetime.today().strftime('%Y%m%d')
	updated_DB_folder = f"DB_{today}"
	os.makedirs(updated_DB_folder, exist_ok=True)

	# ===== merge the sequences into the folder named "DB_(date)" =====
	...
	# ===== count the total number of ORFs =====
	...
	# ===== assign taxonomic information to the qseqid in BLAST alignment results =====
