...
def multipartite_taxonomic_graph_generator(result_dir, vmr_mapping_file, taxon_categories):
	def process_row(row):
		...
		for i, weight in enumerate(weights):
			if row[weight] >= 0.6:
				...
				lineage = ";".join([str(e) for e in row[lineages[i:]]])
				return pd.Series([average, lineage])

		# If all weights are less than 0.6, find the combination with the highest average weight
		total_max_average = -1
			...
			if not current_weights.isna().any():
				...
				if current_average >= 0.95 * total_max_average and current_count >= 3:
					if current_count > max_count:
						...
						max_lineage = ";".join([str(e) for e in row[lineages[-i:]]])
	...
	best_high_score_indices = high_score_df.groupby('Genome_ID')[['dash_count', 'lineage_score']].idxmin(numeric_only=True)
	...
	low_score_df['lineage_score'] = pd.to_numeric(low_score_df['lineage_score'], errors='coerce')
	low_score_df = low_score_df.dropna(subset=['lineage_score'])
...
def assignment():        
	...
	if not Path(input_faa).is_file():
		...
		with open(log_file, "a") as log_file:
			...
			subprocess.run(["pyrodigal", "-p", "meta", "-i", merge_fasta, "-a", input_faa, "-f", "gff", "-o", merge_gff], stdout=log_file, stderr=log_file)
	else:
